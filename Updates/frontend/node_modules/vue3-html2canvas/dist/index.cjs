...
__export(src_exports, {
  Html2CanvasPlugin: () => Html2CanvasPlugin,
  useHtml2Canvas: () => useHtml2Canvas
});
...
// src/plugin.ts
var Html2CanvasPlugin = {
  install: (app) => {
    app.config.globalProperties.$html2canvas = html2canvas_esm_default;
  }
};
...
var useHtml2Canvas = () => {
  const internalInstance = (0, import_vue_demi.getCurrentInstance)();
  const html2canvas2 = internalInstance.appContext.config.globalProperties.$html2canvas;
  async function htmlToCanvas(el, options = {}) {
    const { type } = options;
    const canvas = await html2canvas2(el, options);
