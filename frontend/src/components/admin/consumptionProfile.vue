  ...
  <Card title="اضافه کردن پروفایل مصرف جدید">
    <form class="flex flex-col items-center mt-5 gap-4">
      ...
      <template v-slot:btns>
        <div>
          <button data-modal-hide="popup-modal" aria-label="Close" @click="consumptionProfile" type="button" class="inline-flex justify-center w-full px-2 py-1.5 text-xs font-medium text-center text-white        bg-green-500 hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-green-300  rounded-lg dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800">درسته</button>
        </div>
      </template>
  </modal>
    </form>
