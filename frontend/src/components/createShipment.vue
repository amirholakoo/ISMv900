...
export default {
  ...
  data(){
    return {
      drowpdownList: {
        license_number: {name: 'شماره پلاک',title: 'شماره پلاک', data: '', value: ''},
        supplier_name: {name: 'اسم تامین کننده',title: 'اسم تامین کننده', data: '', value: ''},
        matrial_type: {name: 'نوع ماده',title: 'نوع ماده', data: '', value: ''},
        matrial_name: {name: 'اسم ماده',title: 'اسم ماده', data: '', value: ''},
        shipmet_type: {name: 'نوع بار نامه',title: 'نوع بار نامه', data: '', value: ''},
        customer_name: {name: 'اسم مشتری',title: 'اسم مشتری', data: '', value: ''},
      },
      forms: {
        username: {name: 'نام کاربری', value: ''},
      },
      success: false,
      error: false,
      errors: [],
  ...
  mounted() {
    ...
    this.axios.get('/myapp/api/getLicenseNumbers').then((response) => {
      ...
      this.drowpdownList.license_number.data = response.data['license_numbers']
    })
  ...
  methods:{
    clicked(k, name){
      console.log(k, name)
      if (k == 'license_number'){
        this.drowpdownList.license_number.name = name
        this.drowpdownList.license_number.value = name
      }
    ...
    async addSupplier() {
      const params = {
        "license_number": this.drowpdownList.license_number.value,
...
<template>
 <Card title="افزودن بارنامه">
    <div class="flex flex-col gap-2 justify-center items-center">
      ...
      <div class="relative" v-for="(val, form_name) in forms">
      ...
      <template v-for="(val, dropdown) in drowpdownList">
        ...
        <!-- Dropdown menu -->
        <div :id="dropdown+'dropdown'" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
            <ul class="overflow-y-auto h-48 py-2 text-sm text-gray-700 dark:text-gray-200" :aria-labelledby="dropdown + 'Button'">
              <li v-for="data in val.data">
                <a @click='clicked(dropdown ,data)' type="button" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                  {{ data }}
                </a>
              </li>
            </ul>
      ...
      <modal type="confirm">
        ...
        <template v-slot:text>
          <div class="flex flex-col gap-2 font-bold">
            <template v-for="(val, key) in drowpdownList">
